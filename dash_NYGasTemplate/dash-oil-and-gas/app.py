...
# Load data
device_location_data = pd.read_csv(DATA_PATH.joinpath("device_map.csv"), low_memory=False)
product_code_list = list(device_location_data['product_code'].unique())
state_list = list(device_location_data['state_code'].unique())


product_code_options = [
    {"label": e, "value": str(e)} for e in product_code_list
]
state_options = [
    {"label": e, "value": str(e)} for e in state_list
]
...
points = None # pickle.load(open("/home/ubuntu/dash/dash-oil-and-gas/data/points.pkl", "rb"))
            ...
            [
                html.Div(
                    [
                        ...
                        html.P("Filter by well state:", className="control_label"),
                        dcc.RadioItems(
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.Div(
                                    [html.H6(id="well_text"), html.P("No. of Units")],
                                    id="wells",
                                    className="mini_container",
                                ),
                                html.Div(
                                    [html.H6(id="gasText"), html.P("Alerts")],
                                    id="gas",
                                    className="mini_container",
                                ),
                                html.Div(
                                    [html.H6(id="oilText"), html.P("Ave Tmp")],
                                    id="oil",
                                    className="mini_container",
                                ),
                                html.Div(
...
def update_well_text(well_statuses, well_types, year_slider):
    ...
    df=df.loc[df['product_code'].isin(well_types) & (df['state_code'].isin(well_statuses))]
...
def make_main_figure(
    ...
    fig.update_layout(
        title_text = ' Connected Production Units' + '<br>',
        showlegend = True,
        geo = dict(
            scope = 'usa',
...
def make_aggregate_figure(well_statuses, well_types, year_slider, main_graph_hover):
    ...
    dff = df #filter_dataframe(df, well_statuses, well_types, year_slider)
    ...
    layout_aggregate["title"] = "Aggregate: Temperature"
...
def make_pie_figure(well_statuses, well_types, year_slider):
    ...
    layout_pie["title"] = "Alarm Summary: {} to {}".format(
...
def make_count_figure(well_statuses, well_types, year_slider):
    ...
    dff = df #filter_dataframe(df, well_statuses, well_types, [1960, 2017])
    ...
    layout_count["title"] = "Temperature Diag"
